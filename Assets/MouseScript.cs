...
public class MouseScript : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        Mesh mesh = m_Rigidbody.GetComponent<MeshFilter>().mesh;
        Vector3 size = mesh.bounds.size;
        Ray r = m_Camera.ScreenPointToRay(Input.mousePosition);
        Vector3 hit = r.origin + (r.origin.y/r.direction.y)*r.direction;
        Debug.Log(hit);
        bool xNegative;
        bool yNegative;
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            Vector3 pos = new Vector3(x/size.x +size.x*2.0f*xMultiplier, -size.x*2.0f, y/size.y+size.y*2.0f*yMultiplier);
